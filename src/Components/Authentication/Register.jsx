...
const Register = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!regex.test(password)) {
      setPassError(
        "Password must contain at least one uppercase letter, one lowercase letter, one digit , one special charecter"
      );
      return;
    }
  ...
  return (
    <div className="border border-gray-500 rounded-2xl p-7 w-9/12 mx-auto mt-12 mb-10 bg-cyan-50 grid lg:grid-cols-2 grid-cols-1 gap-7 items-center">
      <div>
        <h2 className="font-bold md:text-2xl text-xl lg:text-3xl text-center">
          User Registration
        </h2>
        <hr className="my-3" />
